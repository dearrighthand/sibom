...
function ConceptPage() {
  ...
  const nextConcept =
    currentIndex < conceptsList.length - 1 ? conceptsList[currentIndex + 1] : null;

  // Get related rules
  const relatedRules = concept.relatedRules?.map((slug) => rules[slug]).filter(Boolean);
  ...
  return (
    <article className="space-y-8">
      ...
      <header className="space-y-4">
        ...
        {concept.description && (
          <p className="max-w-2xl text-lg text-muted-foreground">{concept.description}</p>
        )}
      </header>

      {/* Content */}
      <div className="prose max-w-none" dangerouslySetInnerHTML={{ __html: concept.content }} />
